...
def broyden_method(f, grad_f, x0, tol=1e-8, max_iter=1000):
    ...
    for k in range(max_iter):
        ...
        # Обновление матрицы eta по формуле Пирсона
        try:
            delta_x = x_new - x
            delta_g = grad_f(x_new) - grad_f(x)

            numerator = delta_x - eta @ delta_g
            denom = delta_x @ delta_g
            if abs(denom) > 1e-12:  # Защита от деления на ноль
                delta_eta = np.outer(numerator, delta_x) / denom
                eta = eta + delta_eta
        except:
