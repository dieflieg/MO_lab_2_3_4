...
def broyden_method(f, grad_f, x0, tol=1e-8, max_iter=1000):
    ...
    for k in range(max_iter):
        ...
        # Обновление матрицы eta по третьему алгоритму Пирсона 3
        try:
            delta_x = x_new - x  # Δx^k
            delta_g = grad_f(x_new) - grad_f(x)  # Δg^k

            numerator = delta_x - eta @ delta_g  # [Δx^k - η^k Δg^k]
            denominator = (delta_g.T @ eta @ delta_g)  # (Δg^k)^T η^k Δg^k

            if abs(denominator) > 1e-12:  # Защита от деления на ноль
                delta_eta = np.outer(numerator, eta @ delta_g) / denominator  # Формула третьего алгоритма
                eta = eta + delta_eta
        except:
